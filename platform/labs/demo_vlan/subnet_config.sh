...
subnet_host_router_add () {
  local n_grp="$1" n_router="$2" device="$3" num="$4"

  if [ "$device" = "host" ] ; then

    echo "${n_grp}"".""$(($n_router+101))""."${num}".1/24"

  elif [ "$device" = "router" ] ; then

    echo "${n_grp}"".""$(($n_router+101))""."${num}".2/24"

  elif [ "$device" = "bridge" ] ; then

    echo "${n_grp}"".""$(($n_router+101))""."${num}".0/24"
...
subnet_host_router6() {
  ...
  if [ "$device" = "host" ]; then
    ...
    echo "${n_grp}"":$((200 + $n_router))::1/48"

  elif [ "$device" = "router" ]; then

    echo "${n_grp}"":$((200 + $n_router))::2/48"

  elif [ "$device" = "bridge" ]; then

    echo "${n_grp}"":$((200 + $n_router))::0/48"

  else

    echo "subnet_host_router6: unknown device $device" 1>&2
    exit 1

  fi
}

subnet_host_router_add6 () {
  local n_grp="$1" n_router="$2" device="$3" num="$4"

  if [ "$device" = "host" ] ; then

    echo "${n_grp}"":$((200 + $n_router)):"${num}"::1/48"

  elif [ "$device" = "router" ] ; then

    echo "${n_grp}"":$((200 + $n_router)):"${num}"::2/48"

  elif [ "$device" = "bridge" ] ; then

    echo "${n_grp}"":$((200 + $n_router)):"${num}"::0/48"
...
subnet_netflow_collector () {
  local n_grp="$1" n_router="$2" device="$3"

  if [ "${device}" = "collector" ]; then

    echo "211.0."${n_grp}".$((2*n_router+2))/31"

  elif [ "${device}" = "collector_ip" ]; then

    echo "211.0."${n_grp}".$((2*n_router+2))"

  elif [ "${device}" = "router" ]; then

    echo "211.0."${n_grp}".$((2*n_router+1+2))/31"

  elif [ "${device}" = "main" ]; then

    # echo "211.0."${n_grp}".1/32"
    echo "44.44.44.0/24"

  elif [ "${device}" = "main_ip" ]; then
